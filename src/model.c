...
Texture *model_load_material_textures(Model m, struct aiMaterial *mat, enum aiTextureType type, uint32_t *count);
...
void model_process_node(Model m, struct aiNode *node, const struct aiScene *scene) {
  ...
  for (uint32_t i = 0; i < node->mNumMeshes; i++) {
    ...
    for (uint32_t j = 0; j < m->texture_count; j++) {
      printf("Texture loaded: %s\n", texture_get_path(m->textures_loaded[j]));
    }
  }
...
Mesh model_process_mesh(Model m, struct aiMesh *mesh, const struct aiScene *scene) {
  ...
  if (mesh->mMaterialIndex >= 0) {
    ...
    Texture *diffuse_maps = model_load_material_textures(m, mat, aiTextureType_DIFFUSE, &diffuse_texture_count);
    for (int j = 0; j < m->texture_count; j++)
      printf("==================================\nTexture path loaded %s\n============================================\n", texture_get_path(m->textures_loaded[j]));
    uint32_t specular_texture_count;
    printf("Attempting to load specular texture maps\n");
    Texture *specular_maps = model_load_material_textures(m, mat, aiTextureType_SPECULAR, &specular_texture_count);
    for (int j = 0; j < m->texture_count; j++)
      printf("===================================\nTexture path loaded %s\n============================================\n", texture_get_path(m->textures_loaded[j]));

    for (uint32_t i = 0; i < diffuse_texture_count; i++) {
      textures = realloc(textures, ++num_textures * sizeof(Texture));
      textures[num_textures - 1] = diffuse_maps[i];
    }

    for (uint32_t i = 0; i < specular_texture_count; i++) {
      textures = realloc(textures, ++num_textures * sizeof(Texture));
      textures[num_textures - 1] = specular_maps[i];
    }
...
Texture *model_load_material_textures(Model m, struct aiMaterial *mat, enum aiTextureType type, uint32_t *count) {
  ...
  for (uint32_t i = 0; i < aiGetMaterialTextureCount(mat, type); i++) {
    ...
    for (uint32_t j = 0; j < (m->texture_count); j++) {
      char *p = texture_get_path(m->textures_loaded[j]);
      printf("Texture Path: %s\n", p);
      if (strcmp(p, path) == 0) {
        printf("%s already loaded. Simply copying cached texture into array\n", path);
        textures = realloc(textures, ++tex_count * sizeof(Texture));
        textures[tex_count - 1] = m->textures_loaded[j];
        skip = true;
        break;
      }
    }
    ...
    if (!skip) {
      ...
      printf("Path loading rn %s\n", path);
      texture_create(&tex, 0, path, tex_type);
      textures = realloc(textures, ++tex_count * sizeof(Texture));
      textures[tex_count - 1] = tex;
      printf("texture cache count 1 %d\n", m->texture_count);
      m->textures_loaded = realloc(textures, ++(m->texture_count) * sizeof(Texture));
      m->textures_loaded[m->texture_count - 1] = tex;
